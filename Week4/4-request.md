...
> [!WARNING]
> When accessing a key normally (using get() or []), Flask returns only the first value associated with that key, even if multiple values exist. We can access all values using the `getlist` method.
...
> [!NOTE]
> `request.get_json()` 
> works only when the request’s Content-Type is set to application/json. If the Content-Type is different or the JSON is malformed, it will raise an error. To avoid this, we can use the silent=True
parameter, which returns None instead of raising an error for invalid JSON.
